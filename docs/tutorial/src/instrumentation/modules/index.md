# Semantic Modules

Semantic modules, or mods, are curated vertical slices of Quent's stack. A mod
adds reusable meaning and constraints to basic schema elements, then lets
instrumentation, analysis, and user-interface tooling interpret those elements
consistently.

For example, the Finite-State Machine mod describes the allowed order of an
entity's events, while the Resource mod describes capacity and usage. An
Application Event Schema can combine mods to express the behavior relevant to
that application.
