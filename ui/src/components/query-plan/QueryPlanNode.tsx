...
export const QueryPlanNode = memo(({ data }: { data: QueryPlanNodeData }) => {
  ...
  const nodeContent = (
    <div
      ...
      style={{ zIndex: 10, opacity: isDimmed ? 0.3 : 1, transition: 'opacity 0.15s, transform 0.15s' }}
    >
      {data.hasIncoming && (
        <Handle type="target" position={Position.Top} className="w-2 h-2" style={{ opacity: 0 }} />
      )}

      <div className={`text-sm break-words text-center ${isSelected ? 'font-bold' : 'font-normal'}`}>{data.label}</div>
