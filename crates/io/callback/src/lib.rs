...
pub struct CallbackExporter<S, T> {
    callback: EventCallback<T>,
    source: PhantomData<fn(S)>,
}

impl<S, T> CallbackExporter<S, T> {
...
impl<S, T> Exporter<S> for CallbackExporter<S, T>
where
    ...
    T: Send + 'static,
{
    async fn create_exporter(
        ...
        Ok(Box::new(CallbackExporter::<S, T>::new(self.clone())))
    }
}
