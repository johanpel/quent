...
impl ResourceLowerer {
        ...
        if let Some(capacities) = capacities {
            for (name, capacity) in capacities {
                if let Some(name) = ident(name, &resource_path, sink) {
                    builder =
                        builder.with_capacity(name, Capacity::new(capacity.kind, capacity.bounded));
                }
            }
        }
    ...
    fn usage_record(&self, target: &str, path: &str, sink: &mut Diagnostics) -> Option<Identifier> {
        ...
        ident(target, path, sink).map(|target| ResourceBuilder::default_usage_record_name(&target))
    }
}
...
fn bounds_field(
    ...
    match bounds_record {
        ...
        None => {
            sink.error(
                path,
                "`sets-resource-bounds` needs a resource entity with a bounded capacity",
                None,
            );
            None
        }
