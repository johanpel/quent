...
pub struct Options {
    ...
    /// Generate collector ingestion routing for the model context.
    ///
...
mod path_tests {
    ...
    #[test]
    fn generates_collector_event_routing() {
        let schema = SchemaBuilder::try_new("Demo")
            .unwrap()
        ...
        assert!(source.contains("impl ::quent_instrumentation::Collectible for Demo"));
        assert!(source.contains("::quent_instrumentation::deserialize_event::<"));
        assert!(source.contains("context.observer::<"));
        assert!(source.contains(".send(event)"));
    }

    #[test]
    fn rejects_collector_sink_without_serde() {
        ...
        assert!(matches!(
            ...
            Err(GenerateError::CollectorSinkRequiresSerde)
        ));
    }
}
